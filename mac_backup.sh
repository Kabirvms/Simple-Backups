...
REMOTE_USER="$MAC_BACKUP_USER"
REMOTE_HOST="$MAC_BACKUP_HOST"
REMOTE_STORAGE_LOCATION="$MAC_BACKUP_LOCATION"

# Set up shared variables
GRANDPARENT_DIR="$SCRIPT_DIR"

# Set logs directory dynamically for this backup job
LOGS_DIR="$SCRIPT_DIR/logs/mac_backup"
...
main() {
    ...
    if ! run_pre_backup; then
        ...
        backup_finished 1 "Pre-backup setup failed" "mac"
        exit 1
    fi
    ...
    if ! run_backup_items; then
        ...
        backup_finished 2 "Backup tasks failed" "mac"
        exit 2
    fi
    ...
    if ! run_post_backup; then
        ...
        backup_finished 1 "Post-backup operations failed" "mac"
        exit 1
    fi
    
    backup_finished 0 "Success" "mac"
    exit 0
}
